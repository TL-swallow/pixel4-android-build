...
# 单独检查各项驱动
npm run check-modem      # 检查基带驱动
npm run check-camera     # 检查相机驱动
npm run check-sensors    # 检查传感器驱动
npm run check-audio      # 检查音频驱动
npm run check-display    # 检查显示驱动
```
...
为确保刷机后设备能正常使用，我们提供了全面的驱动检查工具：

#### 🔍 **检查项目**：
- **基带驱动**: 检查移动网络、运营商支持
- **相机驱动**: 检查前后摄像头、闪光灯
- **传感器驱动**: 检查加速度、陀螺仪、磁力计等
- **音频驱动**: 检查扬声器、麦克风、耳机
- **显示驱动**: 检查屏幕、触摸屏、HDR
- **网络驱动**: 检查WiFi、蓝牙、移动网络

#### 🇨🇳 **中国运营商支持**：
- **中国移动**: MCC 460, MNC 00,02,07,08
